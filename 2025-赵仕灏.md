...
```
目前的项目让然会存在albedo分解无效的情况，但重建效果却很好，损失也很平滑（出现坍缩解的情况1.4）
### 研究进度
1. 继续精进albedo分离。
-----------------------------------------------------
## 2025年12月27日 赵仕灏
### 学习进度
1. 本征分解任务，新增shading: (B,1,H,W) 纯光照（不含投影遮挡），visibility: (B,1,H,W) 可见性（1=受光，0=阴影）的参数，将整体光照信息做一个可微的投影阴影
-----------------------------------------------------
## 2026年1月4日 赵仕灏
### 学习进度
1.重新计算Baseline，得到结果：

================ DSM SR Evaluation (Val Set) ================
Samples evaluated: 1326

[ Bicubic ]
RMSE (m): 0.1644 ± 0.1059
MAE  (m): 0.0557 ± 0.0330
RMAE (%): 0.0197 ± 0.0122
Slope RMSE: 0.1088 ± 0.0765
[ Ours ]
RMSE (m): 0.1251 ± 0.0822
MAE  (m): 0.0446 ± 0.0258
RMAE (%): 0.0158 ± 0.0095
Slope RMSE: 0.0784 ± 0.0568

<img width="1872" height="1046" alt="image" src="https://github.com/user-attachments/assets/3d23d7fd-a13c-4d0e-846f-8fe60a7c03be" />
2.加入法向量后定量分析

-----------------------------------------------------
## 2026年1月7日 赵仕灏
### 学习进度（已删除）
1.实现基于Transfomer模型的Dinov3模型实现语义分割，由于dinov3只是一个无监督特征提取器，这里使用了一个2类Kmeans分类器来进行地面与道路的分割项目，但是由于kmenas是一个二类分类器，所以会出现很严重的马赛克（锯齿）效应，下一步是精进分类器模型，让分割的结果更加细致，突出边缘。<img width="1800" height="900" alt="overlay" src="https://github.com/user-attachments/assets/a872f26e-7a2b-45cd-b7c3-3ae0b1b02055" />
2.使用CRF（引导滤波）细化边缘
<img width="1800" height="900" alt="right1" src="https://github.com/user-attachments/assets/d6a5e61a-7679-4762-8b55-2c2d3341ecee" />

-----------------------------------------------------
## 2026年1月21日 赵仕灏
### 学习进度
1. 学习并加入SAM3识别，关键词为building和street,asphalt road,driveway,pavement得到结果，并得到了单波段掩膜
<img width="512" height="512" alt="347ddd4504762029da2b2fad295ca5a9" src="https://github.com/user-attachments/assets/7e28d0b4-c438-4bee-b541-416703bde720" />

-----------------------------------------------------
## 2026年1月24日 赵仕灏
### 学习进度
1. 加入了法向量预测模块，对于建筑内部进行约束，而对于其他部分不进行约束，加入后结果可视化：
   <img width="1550" height="1034" alt="individualImage-2" src="https://github.com/user-attachments/assets/02d86001-98aa-4a99-91ea-44e1e216b31e" />
================ DSM SR Evaluation (Val Set) ================
Samples evaluated: 1326
...
[ Ours ]
RMSE (m): 0.1106 ± 0.0825
MAE  (m): 0.0346 ± 0.0297
RMAE (%): 0.0122 ± 0.0110
Slope RMSE: 0.0641 ± 0.0550
-----------------------------------------------------
## 2026年3月3日 赵仕灏
### 学习进度
1. 对有效区域做mask-aware area downsample，只对有效区域做处理
[ Ours ]
RMSE (m): 0.1088
MAE  (m): 0.0345 
RMAE (%): 0.0122
...
-----------------------------------------------------
## 2026年3月4日 赵仕灏
1. 重新对RGB归一化在训练/评估时做了统一，现在统一使用/255的方式进行了归一化，对所有的数据进行了更新
2. 最近一次的修改将sam3模型改进在扩散过程中，但是效果并不好，所以采取：保留 baseline 主干完全不动，把 sam3_edge 作为额外输入通道喂给 feature extractor，同时在边界附近加一个轻量的 gradient consistency loss，不再直接硬改 cv/ch
3. 阅读论文：Weak supervision makes strong details: fine-grained object recognition in remote sensing images via regional diffusion with VLM，文章实现了遥感图像中细小目标的大模型+深度学习的识别。
